...
(function () {
  ...
  function createBrandCard(brand, pageKind) {
    ...
    addLink('אמזון אנגליה', brand.amazonUk || null, 'brandLink--amazon');
    addLink('אמזון ארה"ב', brand.amazonUs || null, 'brandLink--amazon');
  ...
  function initPage() {
    ...
    fetch(jsonUrl, { cache: 'no-store' })
      ...
      .then(function (brands) {
        ...
        // Intl page: show vegan brands only (data should already be filtered, this is a safety net)
        if (pageKind === 'intl') {
          brands = brands.filter(function (b) { return b.vegan === true; });
        }
